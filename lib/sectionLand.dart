...
class sectionLand extends StatefulWidget {
  List<Lands> ?ListLand;
  sectionLand({Key? key,required this.ListLand}) : super(key: key);

  @override
  State<sectionLand> createState() => _sectionState();
}

class _sectionState extends State<sectionLand> {
  List<Lands> _searchResult = [];

  @override
  void initState() {
    super.initState();
    _searchResult.addAll(widget.ListLand as Iterable<Lands> );
  }

  void _filterList(String searchQuery) {
    _searchResult.clear();
    if (searchQuery.isNotEmpty) {
      widget.ListLand?.forEach((item) {
        if (item.post!.postsable!.location!.contains(searchQuery)
            ||item.post!.postsable!.space!.toString()!.contains(searchQuery)
                ...
                child: Container(
                  ...
                  child: TextField(
                      ...
                      decoration:InputDecoration(
                        ...
                        hintText: " ابحث هنا عن المنطقة أو المساحة",
                          ...
                          Positioned(
                              ...
                              child: InkWell(
                                onTap: () {
                                  ...
                                  print("----$index");
                                  int y=0;
                                  bool x=AppCubit.get(context).listNote3.any((element) {
                                    y=element['id'];
                                    return element['idx'] == index;
                                  });
                                  x?print("false"):print("true");
                                  print("--------$y");
                                  x?AppCubit.get(context).DeleteDataBase3(id:y ):AppCubit.get(context).insertDataBase3(idx: index);
                                },
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children:  [
                            ...
                            Text("المنطقة ${_searchResult[index].post?.postsable?.location} ",style: TextStyle(fontWeight: FontWeight.bold, overflow:TextOverflow.ellipsis ,fontSize: 17.sp)),
                            ...
                            TextButton(
                                onPressed: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (context) => secondryLand(land:  _searchResult[index])));
                                },
