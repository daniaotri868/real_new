...
class SearchHouses extends StatelessWidget {
  List searchHouses;
   SearchHouses({Key? key,required this.searchHouses}) : super(key: key);
                    ...
                    Stack(
                      children: [
                        ClipRRect(
                          ...
                          child:
                          (searchHouses[index].post?.postsable?.images)!.isEmpty?
                          flutterImage.Image.asset('assets/images/empty.png',width: 150.w,height:160.h,fit: BoxFit.cover,)
                              :
                          flutterImage.Image.network('${searchHouses[index].post?.postsable?.images[0].img}'
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children:  [
                          ...
                          Text("العنوان ${searchHouses[index].post?.postsable?.location} ",style: TextStyle(fontWeight: FontWeight.bold, overflow:TextOverflow.ellipsis ,fontSize: 17.sp)),
                          RSizedBox(height: 7.h,),
                          Text("السعر ${searchHouses[index].price} مليون ل.س", style: TextStyle(fontSize: 17.sp)),
                          RSizedBox(height: 7.h,),
                          Text("تاريخ النشر ${formatter.format(searchHouses[index].post!.postDate as DateTime)}",style: TextStyle(color: Colors.grey,fontSize: 15.sp)),
                          RSizedBox(height: 5,),
                          TextButton(
                              onPressed: () {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => second(listH: searchHouses[index],index: index,)));
                              },
