...
class sectionCar extends StatefulWidget {
  List<Cars> ?ListCars;
  sectionCar({Key? key,required this.ListCars}) : super(key: key);

  @override
  State<sectionCar> createState() => _sectionState();
}

class _sectionState extends State<sectionCar> {
  List<Cars> _searchResult = [];

  @override
  void initState() {
    super.initState();
    _searchResult.addAll(widget.ListCars as Iterable<Cars> );
  }

  void _filterList(String searchQuery) {
    _searchResult.clear();
    if (searchQuery.isNotEmpty) {
      widget.ListCars?.forEach((item) {
        if (item.post!.postsable!.name!.contains(searchQuery)
                ...
                child: Container(
                  ...
                  child: TextField(
                      ...
                      decoration:InputDecoration(
                        ...
                        hintText: " ابحث هنا عن الاسم أو الموديل",
                          ...
                          Positioned(
                              ...
                              child: InkWell(
                                onTap: () {
                                  ...
                                  print("--------$y");
                                  x?AppCubit.get(context).DeleteDataBase2(id:y ):AppCubit.get(context).insertDataBase2(idx: index);
                                },
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children:  [
                            ...
                            Text("الاسم ${_searchResult[index].post?.postsable?.name} ",style: TextStyle(fontWeight: FontWeight.bold, overflow:TextOverflow.ellipsis ,fontSize: 17.sp)),
                            ...
                            TextButton(
                                onPressed: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (context) => secondCar(car: _searchResult[index],index: index,)));
                                },
